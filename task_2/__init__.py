...
def creating_session(subsession):
    ...
    for p in subsession.get_players():
        loop_list = list(range(subsession.session.num_groups + 1))
        ## first entry will be zero so we remove it
        del loop_list[0]
        ## loop through group ids
        for i in loop_list:
            if p.participant.group6mem == i:
                t1 = p.participant.pair[0]
                t2 = p.participant.pair[1]
                ## generate new tuple with ids in subsession of players and append them to list pairs to get a new matrix

        ## replace ids in tuples by subsession id of players
        # set new group matrix
